...
public sealed class HiringModule : IModule
{
    ...
    public ModuleManifest Manifest { get; } = new()
    {
        ...
        AgentInstructions =
          ...
          + "Send one entry for EVERY criterion on the rubric and exactly one entry per criterion: "
          + "a set that leaves a criterion out, or that scores the same criterion twice, is refused "
          + "outright and nothing is recorded. Neither is repaired for you — an omitted criterion "
          + "is not marked unresolved on your behalf, because that is a judgement about the "
          + "application that only you can make. The refusal names what is missing or repeated, so "
          + "correct that and send the whole set again. "
